...
public class KCashierTest extends TestCase
{
	//these are instantiated for each test separately via the setUp() method.
	KCashierRole cashier;
...
//		market2 = new MockMarket("mockMarket2");
	...
	public void testWaiterAsksForBill()
	{
		...
		// pre-conditions
		assertEquals("cashier should have zero bills but doesn't", cashier.bills.size(), 0);
		...
		assertEquals("MockWaiter should have an empty event log before the Cashier's scheduler is called. Instead, the MockWaiter's event log reads: "
                + waiter.log.toString(), 0, waiter.log.size());
		assertEquals("cashier got a new bill but doesn't", cashier.bills.size(), 1);
		assertTrue("Cashier should have logged \"Received bill to compute\" but didn't. His log reads instead: " + cashier.log.getLastLoggedEvent().toString(), cashier.log.containsString("Received bill to compute"));
		assertTrue("Cashier's scheduler should have returned true, but didn't.", cashier.pickAndExecuteAnAction());
		assertTrue("Cashier's log should have logged \"computing check and giving it to waiter\" but didn't His log reads instead: " +cashier.log.getLastLoggedEvent().toString(), cashier.log.containsString("computing check and giving it to waiter"));
	...
	public void testOneNormalCustomerScenario()
	{
		...
		// check preconditions
		assertEquals("cashier should have zero bills but doesn't", cashier.bills.size(), 0);
		assertEquals("CashierAgent should have an empty event log before the Cashier's msgBill is called. Instead, the Cashier's event log reads: "
		...
		// customer gives payment to cashier
 		 cashier.msgPayment(10.99, customer, 10.99);
		 ...
		 assertTrue("bills contains a bill with status == givenPayment. It doesn't", cashier.bills.get(0).s == orderState.givenPayment);
